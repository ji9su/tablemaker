...
class Linear(utils):
    ...
    def _analysis(self, dependent, variable, adjustment, type):
        ...
        self.linear_model(formula=formula)
        if type == "categorical":
    ...
    def _simple(self):
        ...
        for dependent in self._variable_dict["dependent"]:
            ...
            if result_df.empty:
                ...
                self._result_df_dict["Simple Linear({dependent})".format(dependent=dependent)] = result_df
    ...
    def _multiple(self):
        ...
        for dependent in self._variable_dict["dependent"]:
            ...
            if result_df.empty:
                ...
                self._result_df_dict["Multiple Linear({dependent})".format(dependent=dependent)] = result_df
            
                vifs = self.collinearity_test()
                self._result_df_dict["Multiple Linear-VIF({dependent})".format(dependent=dependent)] = vifs

    def run(self):

        self._result_df_dict = {}

        if self._sub_method == "Simple Linear Regression":
            self._simple()
        else:
            self._multiple()
